...
st.caption(
    "Envie .html, .htm, .xls, .xlsx, .docx, imagens (jpg/png/gif/bmp/tiff/webp/svg) e/ou PDFs (.pdf). "
...
with st.sidebar:
    ...
    # Versões dos conversores DOCX
    st.write(f"**mammoth:** {_mod_ver('mammoth')}")
    st.write(f"**python-docx:** {_mod_ver('python-docx')}")
    st.write(f"**docx2python:** {_mod_ver('docx2python')}")
...
def _very_simple_html(html: str) -> str:
    ...
    <style>
      ...
      th, td {{ border: 1px solid #999; padding: 6px; word-wrap: break-word; overflow-wrap: anywhere; }}
...
def docx_to_html(uploaded_file) -> str:
    """
    Conversão DOCX com fallback: mammoth -> python-docx -> docx2python.
    Não interrompe o app se os pacotes faltarem; retorna HTML informativo.
    """
    uploaded_file.seek(0)
    raw = uploaded_file.read()
    bio = io.BytesIO(raw)

    # 1) Tenta MAMMOTH (melhor fidelidade e imagens embutidas)
    try:
        import mammoth
        result = mammoth.convert_to_html(bio, convert_image=mammoth.images.img_element(_img_to_data_uri))
        ...
        pass  # segue para o próximo fallback

    # 2) Tenta python-docx (texto com parágrafos e tabelas simples)
    try:
        import docx
        bio.seek(0)
        ...
        for tbl in doc.tables:
            ...
            for row in tbl.rows:
                ...
                for cell in row.cells:
                    cells_html.append(f"<td>{escape(cell.text or '')}</td>")
                rows_html.append("<tr>" + "".join(cells_html) + "</tr>")
            if rows_html:
                parts.append("<table border='1' cellspacing='0' cellpadding='6'>" + "".join(rows_html) + "</table>")

        body = "\n".join(parts) if parts else "<p>(Documento DOCX sem conteúdo legível)</p>"
        return (
            "<html><head><meta charset='utf-8'>"
            "<style>body{font-family:Arial, sans-serif; font-size:12pt; margin:16px;} "
            "table{border-collapse:collapse; width:100%; table-layout:fixed;} "
            "td{border:1px solid #999; padding:6px; word-wrap:break-word; overflow-wrap:anywhere;}</style>"
            "</head><body>" + body + "</body></html>"
        )
    except Exception:
        pass  # segue para o próximo fallback

    # 3) Tenta docx2python (texto corrido)
    try:
        from docx2python import docx2python
        bio.seek(0)
        with docx2python(bio) as doc:
            txt = doc.text or ""
        return (
            "<html><head><meta charset='utf-8'></head>"
            "<body><pre style='white-space:pre-wrap; font-family:Arial, sans-serif; font-size:12pt;'>"
            + escape(txt) +
            "</pre></body></html>"
        )
    except Exception:
        # Último recurso: não interrompe o app
        return (
            "<html><head><meta charset='utf-8'></head>"
            "<body><p>Não foi possível converter o DOCX porque os conversores "
            "(mammoth/python-docx/docx2python) não estão instalados.</p>"
            "<p>Instale pelo menos um: <code>pip install mammoth</code> "
            "ou <code>pip install python-docx</code> ou <code>pip install docx2python</code>.</p></body></html>"
        )
...
def build_pdf_weasy(html_str: str, base_url: str) -> bytes:
    ...
    # CSS de página: margens + numeração (rodapé)
    if preserve_layout:
...
# -----------------------
# Excel -> HTML  (PATCH aplicado: borda #999 e melhorias)
# -----------------------
def excel_to_html(uploaded_file, break_between=True) -> str:
    ...
    for i, sheet in enumerate(xls.sheet_names):
        df = xls.parse(sheet)
        df.columns = [str(c) if c is not None else "" for c in df.columns]  # evita None no cabeçalho
        styled = (
            df.style
...
if uploaded_files:
    ...
    for idx, (name, b) in enumerate(pdfs, start=1):
        st.download_button(
            f"⬇️ Baixar {idx}: {name}.pdf",
            data=b,
            file_name=f"{Path(name).stem}.pdf",
            mime="application/pdf",
            key=f"dl_{idx}"
        )
